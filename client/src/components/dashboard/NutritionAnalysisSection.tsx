...
export const NutritionAnalysisSection: React.FC<NutritionAnalysisSectionProps> = ({ date = new Date() }) => {
  const dateString = date.toISOString().split("T")[0];
  const { data, isLoading, isError } = useNutritionAnalysis(dateString);
    ...
    return (
      <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 animate-pulse h-full">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-white">AI NUTRITION ANALYSIS</h3>
        </div>
  ...
  return (
    <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-700 h-full">
      <div className="flex items-center space-x-2 mb-4">
        <Sparkles className="w-5 h-5 text-cyan-400" />
        <h3 className="text-lg font-semibold text-white tracking-wide">AI NUTRITION ANALYSIS</h3>
      </div>
